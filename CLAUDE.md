...
15. **When a page is expected to provide filtering UX, add page-level filters** in `page.json` using `filterConfig` (schema 2.1.0+) or `filters` (schema 1.0.0) — matching the project's existing format. Pages that do not need filtering may omit `filterConfig`/`filters`, consistent with existing schema 2.1.0 pages in this repo. For externally-authored PBIR, page-level filters are the most reliable way to ensure filters render in the Filter Pane (right sidebar). Canvas slicer visuals are OPTIONAL supplements — do not rely on canvas slicers as the sole filtering mechanism when reliable filtering UX is required, because slicers created outside PBI Desktop may fail to render. When adding filtering to a page: (a) FIRST add `filterConfig`/`filters` to `page.json`, (b) THEN optionally add slicer `visual.json` files for on-canvas convenience. See SKILL.md for the correct JSON format.

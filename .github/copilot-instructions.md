...
- **MANDATORY — after every open or restart**: Power BI Desktop always opens with empty/stale data. First run **exactly one** launcher action, then run refresh:
  ...
  2. If you used `Restart-PBIDesktop.ps1`, wait until Power BI Desktop is fully loaded before refreshing. `Restart-PBIDesktop.ps1` does **not** wait for the Analysis Services port to become available. Poll `Find-PBIDesktopPort.ps1` until it returns a port before proceeding.
  3. Refresh action:

...
16. **When a new page needs filtering controls, prefer page-level filters** in `page.json` using `filterConfig` (schema 2.1.0+) or `filters` (schema 1.0.0), matching the project's existing format. Do not add empty or arbitrary page-level filters to pages that do not require filtering. For externally-authored PBIR, page-level filters are the most reliable way to ensure filtering appears in the Filter Pane (right sidebar). Canvas slicer visuals are OPTIONAL supplements; when a page requires filtering, do not rely on externally-authored slicers as the sole filtering mechanism because they may fail to render. When adding filtering to a page: (a) FIRST add `filterConfig`/`filters` to `page.json`, (b) THEN optionally add slicer `visual.json` files for on-canvas convenience. See SKILL.md for the correct JSON format.
